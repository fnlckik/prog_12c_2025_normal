...
namespace Names
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // F1 - 2 pont
            var a = names.Any(x => x.First < x.Last);
            Console.WriteLine("A: " + a);


            // F2 - 3 pont
            var b = names.Where(x => x.IsMale && x.NameText.StartsWith("B")).Select(x => x.NameText);
            Print("\nB: ", b);
            ...
            //var elso = names.Where(x => x.First == names.Max(y => y.First));
            //var masodik = names.Where(x => x.Last == names.Max(y => y.Last));
            //var harmadik = names.Where(x => x.First + x.Last == names.Max(y => y.First + y.Last));
            //Print("\nElso: ", elso);
            //Print("\nMasodik: ", masodik);
            //Print("\nHarmadik: ", harmadik);
